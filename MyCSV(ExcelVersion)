...
def main():
    ...
    print(BARRA_SEPARADORA)
    print()
    print('ESTE PROGRAMA TEM POR FINALIDADE CRIAR, CARREGAR E ATUALIZAR UMA PLANILHA CUJO FORMATO DO ARQUIVO É ".csv".\n\nNota: Caso queira acessar uma planilha já existente, é necessário que a mesma não esteja aberta.')
    print('Observação: Esta versão possibilita criar e editar planilhas e depois acessá-las no Excel. Não é recomendá-\nvel abrir os arquivos deste programa em outro aplicativo que não seja o Excel (capaz de perder toda plani-\nlha).')
    print('\nATENÇÃO: Não usar o caractere ";" em nenhum momento. Não mover a planilha da pasta "Planilhas-MyCSV(Excel)"" para \noutra!')
    print(BARRA_SEPARADORA)
